...
import pandas as pd
import numpy as np
from src.knn_classifier import (read_csv, train_knn_classifier, evaluate_model)
...
def test_train_knn_classifier():
    """
    Test if train_knn_classifier runs successfully.
    """
    X = np.array([[1, 2], [3, 4], [5, 6]])
    y = np.array([0, 1, 1])

    model = train_knn_classifier(X, y, k=3)
